...
module.exports = {
    ...
    async execute(role) {
        try {
            ...
            const timeWindow = config.timeWindow || 60000; // دقيقة واحدة
            ...
            if (isRateLimited) {
                ...
                // حذف الرتبة المنشأة
                try {
                    await role.delete('حماية ضد إنشاء الرتب المفرط');
                } catch (deleteError) {
                    console.error('خطأ في حذف الرتبة:', deleteError);
                }

                // تنفيذ الإجراء على المنفذ
                if (executorMember) {
                    const reason = `إنشاء رتب مفرط - تم إنشاء أكثر من ${maxRoles} رتب في ${timeWindow / 1000} ثانية`;
                ...
                // تسجيل في السجل
                await this.client.logger.warn(`تم اكتشاف إنشاء رتب مفرط من ${executor.tag} في ${guild.name}`);
            }
    ...
    async sendAlert(guild, executor, role, maxRoles, timeWindow) {
        try {
            ...
            const embed = this.client.logger.createProtectionEmbed(
                'حماية الرتب',
                executor,
                `إنشاء رتب مفرط - تم إنشاء أكثر من ${maxRoles} رتب في ${timeWindow / 1000} ثانية`,
                {
                    target: `الرتبة: ${role.name} (${role.id})`,
                    count: maxRoles
                }
            );
            ...
            console.error('خطأ في إرسال تنبيه حماية الرتب:', error);
        }
    }
};
